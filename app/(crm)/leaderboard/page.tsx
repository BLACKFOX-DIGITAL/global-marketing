...
interface LeaderboardEntry {
    ...
    xp: number; periodXp: number; level: number; title: string
...
export default function LeaderboardPage() {
    ...
    const { data, isLoading } = useSWR<LeaderboardData>(`/api/leaderboard?period=${period}`, fetcher, { keepPreviousData: true })
                                        ...
                                        return (
                                            <div key={entry.id} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flex: 1, maxWidth: 200 }}>
                                                ...
                                                <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 12 }}>
                                                    <span style={{ fontSize: 12, color: rankColor, fontWeight: 700 }}>{entry.periodXp} XP</span>
                                                ...
                                                <div style={{
                                                    ...
                                                    <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>{entry.tasksCompleted} tasks</div>
                                    ...
                                    <table className="data-table">
                                        ...
                                        <tbody>
                                            {data?.leaderboard.map(entry => {
                                                const isCurrentUser = entry.id === cu?.id
                                                return (
                                                    <tr key={entry.id} style={{
                                                        background: isCurrentUser ? 'rgba(99,102,241,0.08)' : undefined,
                                                        borderLeft: isCurrentUser ? '3px solid var(--accent-primary)' : '3px solid transparent',
                                                    }}>
                                                            ...
                                                            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                                                                ...
                                                                <div>
                                                                    <div style={{ fontWeight: 600, fontSize: 13 }}>
                                                                        {entry.name}
                                                                        {isCurrentUser && <span style={{ marginLeft: 6, fontSize: 10, color: 'var(--accent-primary)', fontWeight: 700 }}>YOU</span>}
                                                                    </div>
                                                        ...
                                                        <td style={{ fontSize: 13, fontWeight: 700 }}>{entry.periodXp.toLocaleString()}</td>
                                                        <td>
                                                        ...
                                                        <td style={{ fontSize: 13 }}>{entry.leads}</td>
                                                        <td>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                    {cu && (
                        <div style={{
                            ...
                            <div style={{ fontSize: 12, opacity: 0.7, marginBottom: 16 }}>Level {cu.level} · {cu.xp.toLocaleString()} XP total</div>
                            {cu.rank > 1 && (
                                <div style={{ fontSize: 12, opacity: 0.9, marginBottom: 16, background: 'rgba(255,255,255,0.1)', borderRadius: 8, padding: '8px 12px' }}>
                                    You need <strong>{(data?.leaderboard[cu.rank - 2]?.periodXp || 0) - cu.periodXp} more XP</strong> to reach #{cu.rank - 1}!
                                </div>
                            )}
                            {cu.currentStreak >= 1 && (
