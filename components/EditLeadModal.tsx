...
export default function EditLeadModal({ id, onSuccess, onClose }: { id: string, onSuccess: () => void, onClose: () => void }) {
    ...
    useEffect(() => {
        fetch(`/api/leads/${id}`).then(r => r.json()).then((lead: {
            socials: string, company: string, website: string, country: string, email: string, phone: string,
            status: string, priority: string, industry: string, notes: string, ownerId: string,
            ...
            setOriginalWebsite(lead.website || '')
            if (lead.notes) setShowNotes(true)
    ...
    useEffect(() => {
        if (!form.website || form.website.toLowerCase() === originalWebsite.toLowerCase()) {
            setWebsiteError('')
            return
        }
                                                        ...
                                                        {contact.socials.map((social, sIdx) => (
                                                            <div key={sIdx} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                                                                ...
                                                                <button type="button" onClick={() => {
                                                                    const n = [...contacts]; n[i].socials = n[i].socials.filter((_s, idx: number) => idx !== sIdx); setContacts(n)
